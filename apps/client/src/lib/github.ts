...
export const authCookie = {
  set: (res: NextResponse, token: string, isDev: boolean = IS_DEV_ENV) => {
    res.cookies.set({
      name: "access_token",
      value: token,
      httpOnly: true,
  ...
  async callback(code: string) {
    try {
      ...
      const res = NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_BASE_URL}/oauth/github?status=success`
      );
      authCookie.set(res, data.access_token); // set cookie in response
      return res;
    } catch (error) {
      return NextResponse.json(
        { error: "Authentication failed", description: String(error) },
        { status: 500 }
      );
    }
  },
