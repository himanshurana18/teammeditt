...
export function EditorConfig({ monaco, editor, className }: EditorConfigProps) {
  ...
  // --- FILE IMPORT REF (Optional, commented for simplicity) ---
  const fileInputRef = useRef<HTMLInputElement>(null);
  ...
  useEffect(() => {
    ...
    // --- Internal Monaco _values merge (Optional, commented) ---
    /*
    ...
    const mergedSettings = optionKeys.reduce((acc, key, index) => {
      if (key in storedSettings) return acc;
      const value = currentValues[index];
  ...
  const editorOptions = useMemo(() => {
    ...
    Object.keys(monaco.editor.EditorOption)
      ...
      .forEach((key, index) => {
        ...
        // --- Object with .enabled (Optional) ---
        /*
        if (typeof value === "object" && value !== null && "enabled" in value) {
          options[`${key}.enabled`] = { title: key, type: "boolean", currentValue: value.enabled };
          return;
        }
        */

        if (typeof value === "boolean") {
          options[key] = { title: key, type: "boolean", currentValue: value };
        } else if (typeof value === "number") {
          options[key] = { title: key, type: "number", currentValue: value };
        } else {
          options[key] = {
            title: key,
            type: "text",
  ...
  const filteredSettings = useMemo(() => {
    const searchLower = search.toLowerCase();
    return Object.entries(editorOptions).filter(([_, option]) =>
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      {/* --- Export / Import Buttons Optional, Commented --- */}
      {/*
      <Button onClick={handleImportClick}>Import</Button>
      <Button onClick={() => exportSettings(settings)}>Export</Button>
